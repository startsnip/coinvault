...
$title = "CIT 336 Teaching Presentation";
?>
...
<body>
  ...
  <main class="grid presentation">
    <div class="unit whole align-center">
      <h1>CIT 336 - SQL Joins</h1>
      <p>Connecting tables since 1979</p>
    </div>
    <div class="unit two-thirds">
      <p>We often want to gather data which pertains to more that one table. 
        To do this, we use the <code>JOIN</code> operator in an SQL statement.</p>
        <p>a <code>SQL JOIN</code> connects a table with another table based on the 
          specified primary or foreign keys. As an example, we might want to combine
          a users table with an orders table when the CustomerID primary key in the 
          users table matches the CustomerID foreign key in the Orders table.<p>
          <p>The first of the 4 <code>JOIN</code>s we will be discussing is the
          ...
          <div class="unit two-thirds">
            ...
            <p>An <code>INNER JOIN</code> will only return a row of data where there is
              at least one matching entry in <b>both</b> tables.</p>
              ...
              <ul>
                ...
                <li>An entry is created in the users table with a CustomerID of 223 to 
                  represent Sandra, and a CustomerID of 224 to represent Matthew</li>
                  ...
                  <li>As this script uses an <code>INNER JOIN</code>, Robertson will see 
                    an entry for Sandra, but not for Matthew</li>
                  </ul>
                ...
                <div class="unit two-thirds">
                  ...
                  <p>Each <code>JOIN</code> includes two tables. One before the JOIN clause, and 
                    one After.</p>
                    <p>The table listed before the <code>JOIN</code> is considered the left table.
                      The table listed after the <code>JOIN</code> is considered the right table.
                      <p>A <code>LEFT JOIN</code> will return a row of data for every entry in the 
                        left table, and will also return any matched data from the right table</p>
                        <p>A <code>RIGHT JOIN</code> will return a row of data for every entry in the 
                          right table, and will also return any matched data from the left table</p>
                          <h3>Example</h3>
                          <ul>
                            <li>Actress Sandra Bullock creates an account</li>
                            <li>Actor Matthew Perry also creates an account</li>
                            ...
                            <div class="unit two-thirds">
                              ...
                              <p>A <code>FULL OUTER JOIN</code> will return all rows for both tables, 
                                and will combine data where there is a match.</p>
                                <ul>
                                  <li>Actress Sandra Bullock creates an account</li>
                                  <li>Actor Matthew Perry also creates an account</li>
                                  <li>An entry is created in the users table with a CustomerID of 223 to 
